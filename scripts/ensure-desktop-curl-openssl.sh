#!/usr/bin/env bash
# 桌面 libmpv 网络栈：保证 PREFIX 有 OpenSSL（或系统 SecureTransport）+ libcurl，
# 供 FFmpeg HTTPS 与 mpv -Dlibcurl=enabled。Windows MinGW 无系统 curl 时源码编进前缀。
set -euo pipefail
...
# Windows：优先用桌面构建的 kotv pkg-config 包装器。
...
# macOS：系统 SecureTransport + libcurl 即可，不必往 PREFIX 塞 OpenSSL。
if [[ "$(uname -s 2>/dev/null)" == "Darwin" ]]; then
  if have_curl_pc || [[ -f /usr/lib/libcurl.4.dylib ]] || [[ -f /usr/lib/libcurl.dylib ]]; then
    echo "ok macOS network: system libcurl (+ FFmpeg will use SecureTransport)"
    exit 0
  fi
  echo "ERROR: macOS missing libcurl" >&2
  exit 1
fi

# Linux：优先系统开发包（CI apt）；没有再编进 PREFIX。
...
build_openssl() {
  ...
  if kotv_is_windows_build; then
    perl Configure mingw64 no-shared no-tests no-zlib --prefix="$pref" --libdir=lib
    mingw32-make -j"$JOBS" || make -j"$JOBS"
    mingw32-make install_sw || make install_sw
  else
    ./Configure linux-x86_64 no-shared no-tests no-zlib --prefix="$pref" --libdir=lib
    make -j"$JOBS"
    make install_sw
  fi
  # 部分旧 meson 只认 openssl.pc
...
build_curl() {
  ...
  if kotv_is_windows_build; then
    cfg+=(--host=x86_64-w64-mingw32)
  fi
  ./configure "${cfg[@]}"
  local MAKE="${KOTV_MAKE:-make}"
  if kotv_is_windows_build; then
