...
@interface KotvMpvIosTexture : NSObject <FlutterTexture>
@property(nonatomic, assign) int64_t textureId;
@property(nonatomic, weak) id<FlutterTextureRegistry> registry;
@end

@implementation KotvMpvIosTexture {
  std::mutex _mu;
...
@interface KotvMpvIosStreamHandler : NSObject <FlutterStreamHandler>
@property(nonatomic, copy) FlutterEventSink sink;
@end

@implementation KotvMpvIosStreamHandler
- (FlutterError*)onListenWithArguments:(id)arguments eventSink:(FlutterEventSink)events {
  self.sink = events;
  kotv_mpv_desktop_set_event_cb(
      [](const char* json, void* user) {
        KotvMpvIosStreamHandler* h = (__bridge KotvMpvIosStreamHandler*)user;
...
KotvMpvIosTexture* g_texture = nil;
std::thread g_tick;
...
static void HandleMethod(FlutterMethodCall* call, FlutterResult result) {
  ...
  if ([method isEqualToString:@"create"]) {
    if (!g_texture && g_tex_registry) {
      g_texture = [[KotvMpvIosTexture alloc] initWithRegistry:g_tex_registry];
    }
    char* lib = kotv_find_libmpv_path();
    if (!lib) {
      result([FlutterError errorWithCode:@"NO_LIBMPV"
                                 message:@"libmpv not found; bundle Frameworks/libmpv.dylib (P2)"
                                 details:nil]);
      return;
    }
    int gpu_next = [args[@"gpuNext"] boolValue] ? 1 : 0;
    int vulkan = [args[@"vulkan"] boolValue] ? 1 : 0;
    NSString* hwdec = args[@"decode"] ?: @"auto";
    kotv_mpv_set_preinit_options(gpu_next, vulkan, hwdec.UTF8String);
    int rc = kotv_mpv_desktop_init(lib);
    free(lib);
    if (rc != 0) {
      result([FlutterError errorWithCode:@"CREATE_FAILED"
                                 message:[NSString stringWithFormat:@"libmpv load failed (rc=%d)", rc]
                                 details:nil]);
      return;
    }
    ...
    result(@{
      @"ok" : @YES,
      @"ready" : @YES,
      @"textureId" : @(g_texture ? g_texture.textureId : (int64_t)-1),
    });
    return;
  }
...
extern "C" void kotv_mpv_plugin_register_ios(FlutterEngine* engine) {
  if (!engine) return;
  ...
  g_stream_handler = [KotvMpvIosStreamHandler new];
